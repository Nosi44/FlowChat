...
def generate_reply(speaker):
    ...
    personalities = {
        "Кастет": "все время возмущяется. очень горидться собой. плюет перед собой под ноги. следит за текущим временем",
        "Хлюпик": "постоянно жалуется на жизнь. ненавидит анекдоты. страдает клаустрофобией. чихает. кашляет. плохо видит"
    }
    ...
    - разнообразь тему разговора.
    - дополняйсказанное тем что где то слышал.
